...
class Admin extends User{
    ...
    public function showAddBookForm() {
		...
		<form action"'.htmlspecialchars($_SERVER["PHP_SELF"]).'" method="post">
			<table>
				...
				<tr>
                                    <td>Autor:</td>
                                    <td><input type="text" value="'.@$this->controller->clear($_POST['author']).'" name="author" placeholder="Imie Nazwisko," required/></td>
                                </tr>
                                <tr>
                                    <td>Tłumacz:</td>
                                    <td><input type="text" value="'.@$this->controller->clear($_POST['translator']).'" name="translator" placeholder="Imie Nazwisko,"/></td>
                                </tr>
			</table>
    ...
    public function showBorrow($borrowID){
        ...
        $borrow = $borrow.'<p>Data wypożyczenia: '.$this->controller->clear($rowBorrow['borrow_date_borrow']).'<br>Data zwrotu: '.$this->controller->clear($rowBorrow['borrow_return']).'<br>Odebrano: '.$this->controller->clear($rowBorrow['borrow_received']).'<br>Opóźnienie: '.$delay.'<br>Do zapłaty: '.$delay*0.25.'</p>';
        $borrow = $borrow.'<p><button id="receive">Odebrano</button> <button id="delete">Zwrócono</button></p>';
        $borrow = $borrow.'<div id="reader" align="center">Czytelnik:<br>'.$this->showReaderLight($this->controller->clear($rowBorrow['borrow_reader_id'])).'</div>';
        $borrow = $borrow.'<div id="book" align="center">Książka:<br>'.$this->showBookLight($this->controller->clear($rowBorrow['borrow_book_id'])).'</div>';
        $this->controller->close();
        return $borrow;
    }    
    ...
    public function showAllBooks() {
			...
			while($row = mysqli_fetch_assoc($result)) {
				...
				$books = $books.'<tr onClick="location.href=\'https://torus.uck.pk.edu.pl/~dslusarz/Library/UserAction/book.php?book='.$this->controller->clear($row['book_id']).'\'" /> '
                                                    ...
                                                    . '<td>'.$this->controller->clear($row['publisher_house_name']).'</td>'
                                                    . '<td>'.$this->controller->clear($row['book_nr_page']).'</td>'
    ...
    public function editBook($id,$isbn, $title, $publisher_house, $nr_page, $edition, $premiere, $number, $author){
        $this->controller->connect();
        $row = $this->addPublisherHouse($this->controller->clear($publisher_house));
        $this->controller->deleteTableWhere(false,"authors_books", array(array("book_id","=",$id,"")));
        $this->addAuthors($author, $id);
